...
class Merge:
    ...
    def _StitchImage(self,img1,img2):#img2 rotate to img1 :like imglist[7]
        ...
        newBimg,newGimg,newRimg = map(cv2.equalizeHist,(newBimg,newGimg,newRimg))
    ...
    def _MergeSingleChannle(self,img1,img2,Homo):
        ...
        #newImg = self.MergetestAlg2(result,img1,Point,Spe_point)
        return newImg
    def MergetestAlg2(self,RotateImage,NotRotateImage,point,Spe_point):
        rows_2,cols_2 = NotRotateImage.shape#NotRotateImage是未旋转的图像
        NotRotateImagePoint = [(0,0),(cols_2-1,0),(0,rows_2-1),(cols_2-1,rows_2-1)]
        TotalPoint = point + NotRotateImagePoint
        minX = np.inf
        ...
        for i in range(shape_Rows):
            weight = np.int(((end - start) * 0.1)/2)
            if weight == 0 or minRoute[i] < weight:
                DiffGraph[i,minRoute[i]:] = 1
                temp_Diff[i,minRoute[i]:] = 1
                continue
            weights = np.linspace(0,2*weight,2*weight+1)/(2*weight)
            weights = np.power(weights,5)/(np.power(weights,5)+(1-np.power(weights,5)))
            temp_Diff[i,minRoute[i]+weight+1:] = 1
            # print("weights:")
            # print(weights)
            # print(weight)
            # print(minRoute[i])
            temp_Diff[i,minRoute[i]-weight:minRoute[i]+weight+1] = weights
            DiffGraph[i,minRoute[i]:] = 1
        #假设temp_img在左边
        newMatrix1 = (mask1|mask2)*(DiffGraph == 1)*(1-mask2)
        temp_Diff = temp_Diff * (1 - newMatrix1).astype(np.uint8)*1
        newMatrix2 = (mask1|mask2)*(DiffGraph==0)*(1-mask1)
        temp_Diff = temp_Diff + (newMatrix2).astype(np.uint8)
        temp_Diff[temp_Diff>1] = 1
